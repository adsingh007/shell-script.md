...
| 2 | Declare variable into the script so that it can work for any user or form any path | <ol> <p> WGET="/usr/bin/wget"</br>ECHO="/usr/bin/echo"</br>AWK="/usr/bin/awk"</br>CAT="/usr/bin/cat"</br>PWD="/usr/bin/pwd" </p> </ol> | Script should run without any error | All variable passed script run successfully | Pass |
